/*
 * ============================================================================
 * Module      : dvi_tx_tmds_enc.v
 * Description : TMDS 8b/10b Encoder for DVI/HDMI Transmission.
 * Implements the Transition-Minimized Differential Signaling algorithm to 
 * convert 8-bit video data into 10-bit symbols for high-speed serial links.
 *
 * Architecture:
 * - Stage 1: Transition Minimization (XOR/XNOR reduction).
 * - Stage 2: DC Balancing (Disparity tracking to maintain 0V average).
 * - Control: Encoding of HSync/VSync during Blanking periods (den=0).
 * ============================================================================
 * IO Description:
 * - clock      : Pixel clock.
 * - reset      : System reset.
 * - den        : Data Enable (High = Active Video, Low = Blanking/Sync).
 * - data [7:0] : Raw 8-bit color component (Red, Green, or Blue).
 * - ctrl [1:0] : Control bits (e.g., HSync/VSync or Packet Headers).
 * - tmds [9:0] : Encoded 10-bit symbol ready for serialization.
 * ============================================================================
 * Technical Notes:
 * 1. DC Balancing: The 'cnt_q' register tracks the running disparity (the 
 * cumulative difference between 1s and 0s) to prevent DC bias on the cable.
 * 2. Transition Minimization: Reduces EMI by minimizing the number of 
 * bit transitions in the high-speed differential pair.
 * 3. Latency: This module introduces a small pipeline delay (registered output).
 *
